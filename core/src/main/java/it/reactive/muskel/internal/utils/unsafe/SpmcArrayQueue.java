...
abstract class SpmcArrayQueueProducerField<E> extends SpmcArrayQueueL1Pad<E> {
    protected final static long P_INDEX_OFFSET = UnsafeAccess.addressOf(
	    SpmcArrayQueueProducerField.class, "producerIndex");
...
abstract class SpmcArrayQueueConsumerField<E> extends SpmcArrayQueueL2Pad<E> {
    protected final static long C_INDEX_OFFSET = UnsafeAccess.addressOf(
	    SpmcArrayQueueConsumerField.class, "consumerIndex");
...
abstract class SpmcArrayQueueProducerIndexCacheField<E> extends
	SpmcArrayQueueMidPad<E> {
    // This is separated from the consumerIndex which will be highly contended
    // in the hope that this value spends most
...
abstract class SpmcArrayQueueL3Pad<E> extends
...
public final class SpmcArrayQueue<E> extends SpmcArrayQueueL3Pad<E> {
	...
	// single producer, so store ordered is valid. It is also required to
	// correctly publish the element
	...
	// consumers are gated on latest visible tail, and so can't see a null
	// value in the queue or overtake
	...
	// store ordered, make sure nulling out is visible. Producer is waiting
	// for this value.
	...
	// This ensures the correctness of this method at least for the consumer
	// thread. Other threads POV is not really
