...
public class OperatorGroupByToList<T, K> implements
	Operator<GroupedMuskelProcessor<K, List<T>>, T> {
    ...
    public Subscriber<? super T> apply(
	    Subscriber<? super GroupedMuskelProcessor<K, List<T>>> t) {
	return new OperatorGroupBySubScriber<>(t, keySelector, sortFunction);
    }

    private static class OperatorGroupBySubScriber<T, K>
	    extends
	    AbstractOperatorGroupBySubScriber<T, K, GroupedMuskelProcessor<K, List<T>>> {
	...
	protected void doOnNext(K k, List<T> list) {
	    super.doOnNext(GroupedMuskelProcessor.just(k, list));
