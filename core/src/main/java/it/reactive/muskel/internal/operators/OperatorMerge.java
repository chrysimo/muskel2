...
import it.reactive.muskel.exceptions.CompositeException;
import it.reactive.muskel.exceptions.MissingBackpressureException;
...
import it.reactive.muskel.internal.subscriptions.CompositeSubscription;
...
/**
 * This class is an optimized version of RxJava for remote invocation
 *
 * @param <T>
 */
public class OperatorMerge<T> implements
	Operator<T, MuskelProcessor<? extends T>> {

    /** Lazy initialization via inner-class holder. */
    private static final class HolderNoDelay {
	/** A singleton instance. */
	static final OperatorMerge<Object> INSTANCE = new OperatorMerge<Object>(
    ...
    /**
     * Creates a new instance of the operator with the given delayError and
     * maxConcurrency settings.
     * 
     * @param delayErrors
     * @param maxConcurrent
     *            the maximum number of concurrent subscriptions or
	...
	MergeSubscriber<T> subscriber = new MergeSubscriber<T>(t, delayErrors,
		maxConcurrent);
	MergeSubscription<T> subscription = new MergeSubscription<T>(subscriber);
	subscriber.producer = subscription;
	return subscriber;
    }

    static final class MergeSubscription<T> extends AtomicLong implements
	    Subscription {
	/** */
	private static final long serialVersionUID = 1L;
	...
	public MergeSubscription(MergeSubscriber<T> subscriber) {
	...
	public void request(long n) {
		...
		long oldValue = BackpressureUtils.getAndAddRequest(this, n);
		subscriber.emit();

		if (oldValue == 0) {
		    long r = Math
			    .min(subscriber.maxConcurrent, RingBuffer.SIZE);
    ...
    /**
     * The subscriber that MergeSubscriber. This is an optimized version of
     * RxJava
     * 
     * @param <T>
     *            the value type
	...
	MergeSubscription<T> producer;

	volatile RingBuffer queue;

	/** Tracks the active subscriptions to sources. */
	volatile CompositeSubscription subscriptions;
	/**
	 * Due to the emission loop, we need to store errors somewhere if
	 * !delayErrors.
	 */
	...
	/**
	 * Used to generate unique InnerSubscriber IDs. Modified from onNext
	 * only.
	 */
	long uniqueId;

	/**
	 * Which was the last InnerSubscriber that emitted? Accessed if emitting
	 * == true.
	 */
	long lastId;
	/**
	 * What was its index in the innerSubscribers array? Accessed if
	 * emitting == true.
	 */
	int lastIndex;
	...
	public void onNext(MuskelProcessor<? extends T> t) {
	    ...
	    if (t instanceof ScalarSynchronousMuskelProcessor) {
		tryEmit(((ScalarSynchronousMuskelProcessor<? extends T>) t)
			.get());
	    } else {
		InnerSubscriber<T> inner = new InnerSubscriber<T>(this,
			uniqueId++);
		addInner(inner);
	...
	void removeInner(InnerSubscriber<T> inner) {
	    RingBuffer q = inner.queue;
	    if (q != null) {
		q.release();
	    }
	    // subscription is non-null here because the very first addInner
	    // will create it before
	    // this can be called
	...
	/**
	 * Tries to emit the value directly to the child if no concurrent
	 * emission is happening at the moment.
	 * <p>
	 * Since the scalar-value queue optimization applies to both the main
	 * source and the inner subscribers, we handle things in a shared
	 * manner.
	 * 
	 * @param subscriber
	 * @param value
	...
	protected void queueScalar(InnerSubscriber<T> subscriber, T value) {
	    /*
	     * If the attempt to make a fast-path emission failed due to lack of
	     * requests or an ongoing emission, enqueue the value and try the
	     * slow emission path.
	     */
	    RingBuffer q = subscriber.queue;
	    if (q == null) {
		q = RingBuffer.getSpscInstance();

		// subscriber.onSubscribe(q);
		subscriber.queue = q;
	    }
	    try {
		q.onNext(SentinelUtils.next(value));
	...
	protected void emitScalar(InnerSubscriber<T> subscriber, T value, long r) {
	    ...
	    /*
	     * In the synchronized block below request(1) we check if there was
	     * a concurrent emission attempt and if there was, we stay in
	     * emission mode and enter the emission loop which will take care
	     * all the queued up state and emission possibilities.
	     */
	    emitLoop();
	}
	...
	void emitLoop() {
		...
		for (;;) {
		    // eagerly check if child unsubscribed or we reached a
		    // terminal state.
		    if (checkTerminate()) {
			skipFinal = true;
			return;
		    }
		    RingBuffer svq = queue;

		    long r = producer.get();
		    boolean unbounded = r == Long.MAX_VALUE;

		    // count the number of 'completed' sources to replenish them
		    // in batches
		    int replenishMain = 0;
			...
			for (;;) {
			    ...
			    while (r > 0) {
				o = svq.poll();
				// eagerly check if child unsubscribed or we
				...
				T v = SentinelUtils.getValue(o);
				// if child throws, report bounce it back
				// immediately
				try {
				    child.onNext(v);
				} catch (Throwable t) {
		    ...
		    /*
		     * We need to read done before innerSubscribers because
		     * innerSubcribers are added before done is set to true. If
		     * it were the other way around, we could read an empty
		     * innerSubscribers, get paused and then read a done flag
		     * but an async producer might have added more subscribers
		     * between the two.
		     */
		    boolean d = done;
		    ...
		    if (n > 0) {
			// let's continue the round-robin emission from last
			// location
			long startId = lastId;
			int index = lastIndex;
			...
			int j = index;
			// loop through all sources once to avoid delaying any
			// new sources too much
			for (int i = 0; i < n; i++) {
			    // eagerly check if child unsubscribed or we reached
			    // a terminal state.
			    if (checkTerminate()) {
				skipFinal = true;
				return;
				...
				// if we run out of requests or queued values,
				// break
				if (r == 0 || o == null) {
				    break;
				}
			    ...
			    RingBuffer innerQueue = is.queue;
			    if (innerDone
				    && (innerQueue == null || innerQueue
					    .isEmpty())) {
				removeInner(is);
				if (checkTerminate()) {
				    skipFinal = true;
			    ...
			    // if we run out of requests, don't try the other
			    // sources
			    if (r == 0) {
				break;
			    }
			...
			// if we run out of requests or just completed a round,
			// save the index and id
			lastIndex = j;
			lastId = inner[j].id;
		    }
		    ...
		    // if one or more inner completed, loop again to see if we
		    // can terminate the whole stream
		    if (innerCompleted) {
			continue;
		    }
	...
	/**
	 * Check if the operator reached some terminal state: child
	 * unsubscribed, an error was reported and we don't delay errors.
	 * 
	 * @return true if the child unsubscribed or there are errors available
	 *         and merge doesn't delay errors.
	 */
    ...
    static final class InnerSubscriber<T> extends AbstractSubscriber<T, T> {
	private static final long serialVersionUID = 1L;
	final MergeSubscriber<T> parent;
	final long id;
	volatile boolean done;
	volatile RingBuffer queue;
	int outstanding = RingBuffer.SIZE;
	static final int limit = RingBuffer.SIZE / 4;
	...
	public void requestMore(long n) {
	    ...
	    outstanding = RingBuffer.SIZE;
	    int k = RingBuffer.SIZE - r;
	    if (k > 0) {
		request(k);
	    }
	}
