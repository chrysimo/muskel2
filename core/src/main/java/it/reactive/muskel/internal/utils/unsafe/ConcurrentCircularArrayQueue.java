...
/**
 * A concurrent access enabling class used by circular array based queues this
 * class exposes an offset computation method along with differently memory
 * fenced load/store methods into the underlying array. The class is pre-padded
 * and the array is padded on either side to help with False sharing prvention.
 * It is expected theat subclasses handle post padding.
 * <p>
 * Offset calculation is separate from access to enable the reuse of a give
 * compute offset.
 * <p>
 * Load/Store methods using a <i>buffer</i> parameter are provided to allow the
 * prevention of final field reload after a LoadLoad barrier.
 * <p>
 * 
 * @author nitsanw
 * 
 ...
 *            the generic type
 */
public abstract class ConcurrentCircularArrayQueue<E> extends
	...
	buffer = (E[]) new Object[(actualCapacity << SPARSE_SHIFT) + BUFFER_PAD
		* 2];
    }
