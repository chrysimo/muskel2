...
import it.reactive.muskel.internal.utils.unsafe.MpmcArrayQueue;
...
public abstract class ObjectPool<T> {
     ...
     *            will be removed.
     */
    private ObjectPool(final int min, final int max,
	 ...
	 * 
	 * @Override public void call() { int size = pool.size(); if (size <
	 * min) { int sizeToBeAdded = max - size; for (int i = 0; i <
	 * sizeToBeAdded; i++) { pool.add(createObject()); } } else if (size >
	 * max) { int sizeToBeRemoved = size - max; for (int i = 0; i <
	 * sizeToBeRemoved; i++) { // pool.pollLast(); pool.poll(); } } }
	 * 
