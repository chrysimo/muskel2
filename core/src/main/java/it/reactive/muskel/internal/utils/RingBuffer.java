...
import it.reactive.muskel.internal.utils.unsafe.SpmcArrayQueue;
import it.reactive.muskel.internal.utils.unsafe.SpscArrayQueue;
import it.reactive.muskel.internal.utils.unsafe.UnsafeAccess;
...
/**
 * This Class is derived from RxJava This assumes Spsc or Spmc usage. This means
 * only a single producer calling the on* methods. This is the Rx contract of an
 * Observer (see http://reactivex.io/documentation/contract.html). Concurrent
 * invocations of on* methods will not be thread-safe.
 */
public class RingBuffer implements Subscription {

    public static RingBuffer getSpscInstance() {
	if (UnsafeAccess.isUnsafeAvailable()) {
	    return new RingBuffer(SPSC_POOL, SIZE);
	} else {
	    return new RingBuffer();
	}
    }

    public static RingBuffer getSpmcInstance() {
	if (UnsafeAccess.isUnsafeAvailable()) {
	    return new RingBuffer(SPMC_POOL, SIZE);
	} else {
	    return new RingBuffer();
	}
    }
    ...
    /**
     * Queue implementation testing that led to current choices of data
     * structures:
     * 
    ...
    /**
     * We store the terminal state separately so it doesn't count against the
     * size. We don't just +1 the size since some of the queues require sizes
     * that are a power of 2. This is a subjective thing ... wanting to keep the
     * size (ie 1024) the actual number of onNext that can be sent rather than
     * something like 1023 onNext + 1 terminal event. It also simplifies
     * checking that we have received only 1 terminal event, as we don't need to
     * peek at the last item or retain a boolean flag.
     */
    public volatile Object terminalState;

    // default size of ring buffer
    /**
     * 128 was chosen as the default based on the numbers below. A stream
     * processing system may benefit from increasing to 512+.
     * 
     * <pre>
     * {@code
     ...
     * </pre>
     */
    static int _size = 100;
    static {
	// possible system property for overriding
	String sizeFromProperty = System.getProperty("rx.ring-buffer.size"); // also
									     // see
									     // IndexedRingBuffer
	if (sizeFromProperty != null) {
		...
		System.err.println("Failed to set 'rx.buffer.size' with value "
    ...
    private RingBuffer(Queue<Object> queue, int size) {
	this.queue = queue;
	this.pool = null;
	this.size = size;
    }

    private RingBuffer(ObjectPool<Queue<Object>> pool, int size) {
	this.pool = pool;
    ...
    /* package accessible for unit tests */RingBuffer() {
	...
	synchronized (this) {
	    Queue<Object> q = queue;
	    if (q != null) {
		mbe = !q.offer(SentinelUtils.next(o));
	...
	if (iae) {
	    throw new IllegalStateException(
		    "This instance has been unsubscribed and the queue is no longer usable.");
	}
	if (mbe) {
	...
	if (terminalState == null) {
	    // terminalState = on.complete();
	    terminalState = SentinelUtils.complete();
	}
    }
	...
	if (terminalState == null) {
	    ...
	    terminalState = SentinelUtils.error(t);
	}
    }
    ...
    /*
     * public boolean isCompleted(Object o) { return on.isCompleted(o); }
     * 
     * public boolean isError(Object o) { return on.isError(o); }
     * 
     * public Object getValue(Object o) { return on.getValue(o); }
     * 
     * @SuppressWarnings({ "unchecked", "rawtypes" }) public boolean
     * accept(Object o, Observer child) { return on.accept(child, o); }
     * 
     * public Throwable asError(Object o) { return on.getError(o); }
     */
