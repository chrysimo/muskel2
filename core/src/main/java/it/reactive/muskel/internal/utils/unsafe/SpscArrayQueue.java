...
abstract class SpscArrayQueueColdField<E> extends
	ConcurrentCircularArrayQueue<E> {
    private static final Integer MAX_LOOK_AHEAD_STEP = Integer.getInteger(
	    "jctools.spsc.max.lookahead.step", 4096);
...
abstract class SpscArrayQueueProducerFields<E> extends SpscArrayQueueL1Pad<E> {
    protected final static long P_INDEX_OFFSET = UnsafeAccess.addressOf(
	    SpscArrayQueueProducerFields.class, "producerIndex");
...
abstract class SpscArrayQueueConsumerField<E> extends SpscArrayQueueL2Pad<E> {
    ...
    protected final static long C_INDEX_OFFSET = UnsafeAccess.addressOf(
	    SpscArrayQueueConsumerField.class, "consumerIndex");
 ...
 * <p>
 * This implementation is a mashup of the <a
 * href="http://sourceforge.net/projects/mc-fastflow/">Fast Flow</a> algorithm
 * with an optimization of the offer method taken from the <a
 * href="http://staff.ustc.edu.cn/~bhua/publications/IJPP_draft.pdf">BQueue</a>
 * algorithm (a variation on Fast Flow), and adjusted to comply with Queue.offer
 * semantics with regards to capacity.<br>
...
public final class SpscArrayQueue<E> extends SpscArrayQueueL3Pad<E> {
	...
	soProducerIndex(index + 1); // ordered store -> atomic and ordered for
				    // size()
	...
	soConsumerIndex(index + 1); // ordered store -> atomic and ordered for
				    // size()
